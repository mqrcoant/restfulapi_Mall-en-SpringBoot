...
import com.marcopc.restfulapi.repository.LocalRepository;
...
public class LocalServiceImpl implements LocalService {
    ...
    public Optional<Local> findLocalByNameJPQL(String name) {
        return localRepository.findByLocalByNameJPQL(name);
    }
    ...
    public Local findLocalById(Long id) throws LocalNotFoundException {
        // Se busca en la bd, se valida y luego se obtiene el local por id
        Optional<Local> local = localRepository.findById(id);
        if(!local.isPresent()) {
            throw new LocalNotFoundException("Local is not on the database");
        }
        return local.get();
    }
}
